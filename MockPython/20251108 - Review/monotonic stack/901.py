...
class StockSpanner:
    ...
    def __init__(self):
        self._decreaseStock = [(0, 0)] # max heap
        return

    def next(self, price: int) -> int:
        span = 1
        while self._decreaseStock and self._decreaseStock[-1][0] <= price:
            _, prev_span = self._decreaseStock.pop()
            span += prev_span
        self._decreaseStock.append((price, span))
        return span
